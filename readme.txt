...
Poker_Sim supports hand evaluation, Monte Carlo simulation, exact enumeration, opponent range parsing, blocker-aware range filtering, and hand-vs-range equity analysis. The project also includes both a readable reference evaluator and an optimized lookup-table evaluator, along with automated correctness tests and performance benchmarks.
...
```text
(wins + ties / 2) / total outcomes
```

The range engine automatically removes impossible opponent combinations using known cards from the hero hand and board.
...
For example, if Hero holds the Ace of Spades, every opponent combination containing the Ace of Spades is removed before simulation.
...
* Short-form card notation such as `As`, `Kh`, and `Td`
